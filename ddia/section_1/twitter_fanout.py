...
class TwitterFanOut(Scene):
    ...
    def scene_title(self):
        ...
        subtitle = make_label(
            "Designing Data-Intensive Applications — Ch. 1", font_size=22, color=GREY_B
        )
    ...
    def scene_problem_setup(self):
        ...
        # Post tweet card
        post_card = make_card(
            "Post Tweet", width=3, height=1, label_color=ORANGE, font_size=22
        )
        ...
        # Home timeline card
        home_card = make_card(
            "Home Timeline", width=3, height=1, label_color=GREEN, font_size=22
        )
        ...
        skew_text = make_label(
            "Reads are ~65x more than writes!", font_size=24, color=RED
        )
    ...
    def scene_approach_pull(self):
        ...
        tables = (
            VGroup(tweets_table, users_table, follows_table)
        ...
        write_arrow = Arrow(
            user_a.circle.get_right(),
            tweets_table.get_left(),
            buff=0.15,
            stroke_width=3,
            color=ORANGE,
            tip_length=0.15,
        ...
        follower = make_user_icon(
            "Follower", color=GREEN_D, image_path="assets/person.png"
        )
        follower.move_to(RIGHT * 5 + UP * 1.5)
        self.play(FadeIn(follower, shift=LEFT * 0.3))

        read_arrow = Arrow(
            follower.circle.get_left(),
            tweets_table.get_right(),
            buff=0.15,
            stroke_width=3,
            color=GREEN,
            tip_length=0.15,
        ...
        join1 = DashedLine(
            tweets_table.get_bottom(),
            users_table.get_top(),
            color=GREY_B,
            stroke_width=1.5,
        )
        join2 = DashedLine(
            users_table.get_bottom(),
            follows_table.get_top(),
            color=GREY_B,
            stroke_width=1.5,
        )
        ...
        # Warning
        warning = make_label(
            "300k reads/sec = heavy JOIN load!", font_size=22, color=RED
        )
    ...
    def scene_approach_push(self):
        ...
        stats2 = make_label(
            "4.6k tweets/s x 75 = 345k writes/s", font_size=18, color=ORANGE
        )
        stats_group = (
            VGroup(stats1, stats2).arrange(DOWN, buff=0.2).to_edge(DOWN, buff=1.2)
        )
        ...
        self.play(
            FadeOut(arrows),
            FadeOut(follower_caches),
            FadeOut(tweet_box),
            FadeOut(stats_group),
        )

        # Celebrity scenario — morph circle, swap text char by char
        celeb = make_user_icon(
            "Taylor Swift", color=YELLOW, font_size=16, image_path="assets/taylor.png"
        )
        celeb.move_to(LEFT * 4.5 + UP * 0.5)
        celeb_tweet = make_card(
            "Tweet",
            width=1.8,
            height=0.6,
            fill_color="#2D333B",
        ...
        many_caches.arrange_in_grid(rows=rows, cols=cols, buff=0.1).move_to(
            RIGHT * 3 + UP * 0.5
        )
        ...
        # Warning text
        celeb_warning = make_label(
            "1 tweet = 79M+ writes to caches!", font_size=24, color=RED
        )
        ...
        delivery = make_label(
            "Twitter aims to deliver within 5 seconds!", font_size=18, color=YELLOW
        )
